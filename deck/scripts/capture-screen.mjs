...
const eyesDir  = path.join(repoRoot, '.github', 'eyes')
const stateFile = path.join(repoRoot, '.github', 'memory', 'state.md')
const EDGE = 'C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe'
const sleep = ms => new Promise(r => setTimeout(r, ms))

// ── CLI + state.md defaults ──
const args = process.argv.slice(2)
const arg = (name, fb) => { const i = args.indexOf(`--${name}`); return i !== -1 && args[i+1] ? args[i+1] : fb }
const numArg = (name, fb) => { const v = arg(name, null); return v != null ? Number(v) : fb }
...
if (!PROJECT) { console.error('❌ No project. Use --project <id> or set it in .github/memory/state.md'); process.exit(1) }
...
async function main() {
  ...
  // Dev server check
  try { const r = await fetch(`http://localhost:${PORT}`, { signal: AbortSignal.timeout(3000) }); if (!r.ok) throw 0 }
  catch { console.error(`❌ Dev server not reachable on port ${PORT}`); process.exit(1) }
  ...
  if (SLIDE && SLIDE > 1) {
    ...
    for (let i = 0; i < total; i++)     { await page.keyboard.press('ArrowLeft');  await sleep(50)  }
    for (let i = 0; i < SLIDE - 1; i++) { await page.keyboard.press('ArrowRight'); await sleep(100) }
    await sleep(800)
  }
