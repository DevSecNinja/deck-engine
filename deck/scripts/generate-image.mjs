...
const imagesDir = path.resolve(root, 'src', 'data', 'generated')

// ── Load .env from deck/.env ──
...
const SIZE    = arg('size', '1024x1024')     // 1024x1024 | 1536x1024 | 1024x1536 | auto
const QUALITY = arg('quality', 'auto')       // low | medium | high | auto
const MODEL   = arg('model', 'chatgpt-image-latest') // fallback: gpt-image-1.5

const API_KEY = process.env.OPENAI_API_KEY
if (!API_KEY) { console.error('❌ Set OPENAI_API_KEY environment variable'); process.exit(1) }
if (!PROMPT)  { console.error('❌ --prompt is required'); process.exit(1) }
if (!NAME)    { console.error('❌ --name is required (used as filename)'); process.exit(1) }
...
async function main() {
  ...
  // Image API returns base64 in data[].b64_json or URL in data[].url
